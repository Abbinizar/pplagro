    ...
    <div class="w3-row-padding">
      <div class="w3-col m12">
        <div class="w3-card w3-round w3-white">
          ...
          @foreach($data as $row)
            <form action="{{url('investor/konfirmasipembayaran')}}/{{$row->id_pembayaran}}" method="post"  enctype="multipart/form-data">
              {{csrf_field()}}
              ...
              <input type="file" accept=".jpg" name="buktitransfer" id="file" class="w3-rest w3-border">
              <br>
              @if ($errors->has('buktitransfer')) <p style="color: red">{{ $errors->first('buktitransfer') }}</p> @endif
